...
$score = 0;
$input = ""; // Replace with the actual input from the user
$solution = ""; // Replace with the correct solution

if ($input == $solution) {
    $score += 1000; // increment by 1000 for correct answer
} else {
    $score -= 200; // decrement by 200 for every incorrect answer
}
  ...
  <script>
    ...
    function handleInput() {
      ...
      if (input.value == solution) {
        ...
        if(score>=100)
        // If the answer is incorrect, the score decrement  by 200
        score -= 200;
        document.getElementById("score").textContent = score;
        // display message for incorrect answer
        message.innerHTML = "Incorrect answer, You have lost in the Quest!";
        message.style.color = 'white';

        // Reduce progress bar length based on the number of incorrect answers
        let reductionFactor = numIncorrect / maxIncorrect; // Reduce by 25% per incorrect answer
...
<div class="questions" align="center">
  <div>
	...
	<button class="button-62" type="exit" onclick="window.location.href='welcome.php'" >Exit</button>
    <div id="message"></div>
  </div>
